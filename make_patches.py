            ...
            for h_i in range(0, h_pixels, int(patch_size * step)):
                ...
                if is_contours:
                    ...
                    for label in np.unique(slide_mask):
                        if label == 0:
                            continue
                        contours, hierarchy = cv2.findContours(np.where(slide_mask == label, 255, 0).astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
                        contour_map = cv2.drawContours(contour_map, contours, -1, color_table[label], 2)

                        color_masked_patch[:, :, 2] = np.where(contour_map.sum(axis=-1) > 0, color_table[label][0], color_masked_patch[:, :, 2])
                        color_masked_patch[:, :, 1] = np.where(contour_map.sum(axis=-1) > 0, color_table[label][1], color_masked_patch[:, :, 1])
                        color_masked_patch[:, :, 0] = np.where(contour_map.sum(axis=-1) > 0, color_table[label][2], color_masked_patch[:, :, 0])
